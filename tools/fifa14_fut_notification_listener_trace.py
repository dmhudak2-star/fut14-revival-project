...
dropped, which matches a submission that never completes.  This hook records
the bus object, the operation id and that slot, executes the displaced
comparison and resumes.  It changes no argument, return value, state flag,
event or route.
"""
...
def build_stub() -> bytes:
    """Record the bus object, the operation and its listener slot.

    Every instruction here is a non-recording form, so the displaced ``cmpwi``
    still sets cr6 for the retail code that follows.  r3 and r4 are the bus
    arguments, leaving r10 to r12 as scratch at this leaf entry.
    """
    words: list[int] = []
    ...
    words.extend(
        (
            ...
            lwz(12, 3, LISTENER_OFFSET),
            stw(12, 11, RECORD_SIZE + 0x8),
            0x7C0004AC,  # sync
...
def read_journal(client: Xbdm) -> None:
    ...
    if not count:
        print("The notification bus was not reached.")
        return
