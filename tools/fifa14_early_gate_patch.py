#!/usr/bin/env python3
"""Patch FIFA 14's EnterFUT gate immediately on the default.xex modload event."""

from __future__ import annotations

import argparse
import re
import select
import socket
import sys
import time


ADDRESS = 0x82835220
ORIGINAL = bytes.fromhex("2B160000")
LEGACY_PATCHED = bytes.fromhex("48000040")
PATCHED = bytes.fromhex("48000020")


class Connection:
    def __init__(self, host: str, timeout: float = 8):
        self.sock = socket.create_connection((host, 730), timeout=timeout)
        self.reader = self.sock.makefile("rb")
        if not self.line().startswith("201-"):
            raise RuntimeError("Unexpected XBDM banner")

    def line(self) -> str:
        raw = self.reader.readline()
        if not raw:
            raise EOFError("XBDM closed the connection")
        return raw.decode("ascii", "replace").rstrip("\r\n")

    def command(self, command: str, expected: int = 200) -> str:
        self.sock.sendall((command + "\r\n").encode("ascii"))
        while True:
            response = self.line()
            match = re.match(r"^(\d{3})[- ]", response)
            if match:
                if int(match.group(1)) != expected:
                    raise RuntimeError(f"{command}: {response}")
                return response

    def multiline(self, command: str) -> list[str]:
        self.sock.sendall((command + "\r\n").encode("ascii"))
        status = self.line()
        if not status.startswith("202-"):
            raise RuntimeError(f"{command}: {status}")
        result = []
        while True:
            line = self.line()
            if line == ".":
                return result
            result.append(line)

    def read(self, address: int, length: int) -> bytes:
        data = bytes.fromhex(
            "".join(
                self.multiline(
                    f"getmem addr=0x{address:08X} length=0x{length:X}"
                )
            )
        )
        if len(data) != length:
            raise RuntimeError(f"Short read at 0x{address:08X}")
        return data

    def write(self, address: int, data: bytes) -> None:
        self.command(
            f"setmem addr=0x{address:08X} data={data.hex().upper()}"
        )

    def close(self) -> None:
        self.reader.close()
        self.sock.close()


def main() -> int:
    parser = argparse.ArgumentParser()
    parser.add_argument("host")
    parser.add_argument("--timeout", type=int, default=180)
    args = parser.parse_args()

    notify = Connection(args.host)
    control: Connection | None = None
    stopped = False
    try:
        notify.command(
            'debugger connect override name="FIFAEarlyPatch" user="CodexMac"'
        )
        notify.command("notify reconnectport=1", expected=205)
        control = Connection(args.host)

        print("Waiting for default.xex to load. Launch FIFA 14 now.", flush=True)
        deadline = time.monotonic() + args.timeout
        while time.monotonic() < deadline:
            readable, _, _ = select.select([notify.sock], [], [], 1)
            if not readable:
                continue
            event = notify.line()
            lowered = event.lower()
            if "modload" not in lowered or 'name="default.xex"' not in lowered:
                continue

            print(f"Module event: {event}", flush=True)
            control.command("stop")
            stopped = True
            before = control.read(ADDRESS, 4)
            print(f"0x{ADDRESS:08X} before: {before.hex().upper()}")
            if before in (ORIGINAL, LEGACY_PATCHED):
                control.write(ADDRESS, PATCHED)
            elif before != PATCHED:
                raise RuntimeError(
                    f"Unexpected instruction: {before.hex().upper()}"
                )
            after = control.read(ADDRESS, 4)
            if after != PATCHED:
                raise RuntimeError(
                    f"Patch verification failed: {after.hex().upper()}"
                )
            print("Verified: gate patched before title execution.")
            control.command("go")
            stopped = False
            print("Execution resumed. Continue to the FIFA menu and test FUT.")
            return 0

        raise TimeoutError("default.xex modload event was not observed")
    finally:
        if control is not None:
            if stopped:
                try:
                    control.command("go")
                    print("Execution resumed during cleanup.")
                except Exception:
                    pass
            control.close()
        notify.close()


if __name__ == "__main__":
    try:
        raise SystemExit(main())
    except KeyboardInterrupt:
        print("\nInterrupted; cleanup attempted.", file=sys.stderr)
        raise SystemExit(130)
    except Exception as error:
        print(f"\nError: {error}", file=sys.stderr)
        raise SystemExit(1)
